...
| TEST-06 | Đóng một phần theo tỷ lệ | **DEMO** | Phase 7: `PAIR-000027` Master 0.05→0.03, Client 0.05→**0.03**, `PARTIALLY_CLOSED`. Chính bài này lộ ra lỗi float và buộc chuyển sang `Decimal`. |
| TEST-07 | Nhiều lệnh cùng symbol, đóng một cái | **DEMO** | Phase 7, `PAIR-000026` là cặp đối chứng cùng symbol không bị đụng. Thêm `test_ba_lenh_dong_cai_o_giua` cho trường hợp ba cặp. |
