...
| TEST-26 | Đóng hẳn qua giao diện | **CHƯA** | Master đóng → deal đóng phía Client có `entry = OUT` và **`reason = 0`**. Cặp đối chứng cùng symbol không bị đụng tới (tra theo `position_id`). |
| TEST-27 | Đóng một phần qua giao diện | **CHƯA** | Master đóng bớt 25% → `CLOSE_UI_PARTIAL` mang đúng volume, Client còn đúng phần tương ứng, `reason = 0`. **Cần một vị thế lớn hơn mức tối thiểu của sàn** — với 0.01 BTCUSD.s thì không đóng một phần được. |
| TEST-28 | Clicker chết giữa chừng, Master đóng | **CHƯA** | Rơi về `OrderSend` của EA: vị thế Client **vẫn đóng được**, alert CRITICAL `CLOSE_FELL_BACK_TO_EA` hiện trên dashboard, `client_close_reason = 3` mà **không** sinh `UI_CLOSE_REASON_MISMATCH` (cú rơi về đã có alert riêng). Đặt `close_degraded_fallback = SKIP` thì không gửi gì và alert `CLOSE_KHONG_GUI_DUOC`. |
