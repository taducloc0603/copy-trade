...
| Lệnh mở bị từ chối, lý do có `Khong thay Market Watch` | Khung Market Watch trên terminal Client đang đóng. Từ D-44 clicker mở lệnh bằng cách nhấp đúp dòng symbol ở đó, không theo chart | Trong terminal Client bấm `Ctrl+M` và **để nguyên khung đó**. Bridge thử lại lệnh bị từ chối vài lần (`max_retry`, cách nhau `retry_interval_ms`) — mở kịp trong lúc đó thì lệnh vẫn đi. Hết lượt thì cặp thành **mở thất bại** kèm alert, và **không** tự mở bù: kiểm và quyết bằng tay. |
| Lệnh mở bị từ chối, lý do có `Khong dong Market Watch nao mo ra <symbol>` | Symbol đó không có dòng trong Market Watch của Client | Chuột phải trong Market Watch → **Symbols** → chọn symbol → **Show**. Symbol đã có ánh xạ thì bình thường đã có sẵn (ánh xạ chỉ lưu được khi symbol nằm trong Market Watch) — thường là ai đó vừa **Hide** nó. |
